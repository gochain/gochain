...
ADD netstats.json /netstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../eth-net-intelligence-api && pm2 start /netstats.json)' >  explorer.sh && \
...
// explorerNetstats is the configuration file for the netstats javascript client.
var explorerNetstats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	netstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerNetstats)).Execute(netstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.netstats[:strings.Index(config.netstats, ":")],
		"Secret": config.netstats[strings.Index(config.netstats, ":")+1 : strings.Index(config.netstats, "@")],
		"Host":   config.netstats[strings.Index(config.netstats, "@")+1:],
	})
	files[filepath.Join(workdir, "netstats.json")] = netstats.Bytes()
