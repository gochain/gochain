...
var (
	...
	meteredPeerFeed  MeteredPeerFeed // Event feed for peer metrics
...
func SubscribeMeteredPeerEvent(ch chan<- MeteredPeerEvent, name string) {
	meteredPeerFeed.Subscribe(ch, name)
}
...
type MeteredPeerFeed struct {
	mu   sync.RWMutex
	subs map[chan<- MeteredPeerEvent]string
}

func (f *MeteredPeerFeed) Close() {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *MeteredPeerFeed) Subscribe(ch chan<- MeteredPeerEvent, name string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.subs == nil {
		f.subs = make(map[chan<- MeteredPeerEvent]string)
	}
	f.subs[ch] = name
}

func (f *MeteredPeerFeed) Unsubscribe(ch chan<- MeteredPeerEvent) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *MeteredPeerFeed) Send(e MeteredPeerEvent) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	for sub, name := range f.subs {
		...
		default:
			...
			log.Warn(fmt.Sprintf("MeteredPeerFeed send %s: channel full", action), "name", name, "cap", cap(sub), "time", dur, "val", e)
		}
	}
}
