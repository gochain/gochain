...
func newTestProtocolManager(ctx context.Context, lightSync bool, blocks int, generator func(context.Context, int, *core.BlockGen), odr *LesOdr, peers *peerSet, db common.Database) (*ProtocolManager, error) {
	var (
	...
	pm, err := NewProtocolManager(gspec.Config, indexConfig, lightSync, NetworkId, evmux, peers, chain, nil, db, odr, nil, nil, make(chan struct{}), new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	if !lightSync {
...
// fails the test.
func newTestProtocolManagerMust(ctx context.Context, t *testing.T, lightSync bool, blocks int, generator func(context.Context, int, *core.BlockGen), odr *LesOdr, peers *peerSet, db common.Database) *ProtocolManager {
	pm, err := newTestProtocolManager(ctx, lightSync, blocks, generator, odr, peers, db)
	if err != nil {
...
func newServerEnv(t *testing.T, blocks int, protocol int, waitIndexers func(*core.ChainIndexer, *core.ChainIndexer, *core.ChainIndexer)) (*TestEntity, func()) {
	...
	pm := newTestProtocolManagerMust(ctx, t, false, blocks, testChainGen, nil, nil, db)
...
func newClientServerEnv(t *testing.T, blocks int, protocol int, waitIndexers func(*core.ChainIndexer, *core.ChainIndexer, *core.ChainIndexer), newPeer bool) (*TestEntity, *TestEntity, func()) {
	...
	pm := newTestProtocolManagerMust(ctx, t, false, blocks, testChainGen, nil, peers, db)
	lpm := newTestProtocolManagerMust(ctx, t, true, 0, nil, odr, lPeers, ldb)
	...
	if newPeer {
		peer, err1, lPeer, err2 = newTestPeerPair(ctx, "peer", protocol, pm, lpm)
		select {
