...
func GetHeaderByNumber(ctx context.Context, odr OdrBackend, number uint64) (*types.Header, error) {
	...
	if (hash != common.Hash{}) {
		...
		header := core.GetHeader(db.HeaderTable(), hash, number)
		if header == nil {
...
func GetBodyRLP(ctx context.Context, odr OdrBackend, hash common.Hash, number uint64) (rlp.RawValue, error) {
	if data := core.GetBodyRLP(odr.Database().BodyTable(), hash, number); data != nil {
		return data, nil
	}
...
func GetBlock(ctx context.Context, odr OdrBackend, hash common.Hash, number uint64) (*types.Block, error) {
	...
	header := core.GetHeader(odr.Database().HeaderTable(), hash, number)
	if header == nil {
...
func GetBlockReceipts(ctx context.Context, odr OdrBackend, hash common.Hash, number uint64) (types.Receipts, error) {
	receipts := core.GetBlockReceipts(odr.Database().ReceiptTable(), hash, number)
	if receipts != nil {
		return receipts, nil
	}
...
func GetBloomBits(ctx context.Context, odr OdrBackend, bitIdx uint, sectionIdxList []uint64) ([][]byte, error) {
	...
	for i, sectionIdx := range sectionIdxList {
		...
		bloomBits, err := core.GetBloomBits(db.GlobalTable(), bitIdx, sectionIdx, sectionHead)
		if err == nil {
			result[i] = bloomBits
		} else {
