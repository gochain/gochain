...
type Downloader struct {
	...
	stateDB    common.Database // Database to state sync into (and deduplicate via)
...
func New(checkpoint uint64, stateDb common.Database, stateBloom *trie.SyncBloom, mux *core.InterfaceFeed, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
	...
	if d.mode == FastSync {
		...
		if origin+1 < frozen {
			...
			for i := origin + 1; i < d.lightchain.CurrentHeader().Number.Uint64(); i++ {
				hashes = append(hashes, rawdb.ReadCanonicalHash(d.stateDB.HeaderTable(), i))
			}
