...
func (dl *downloadTester) FastSyncCommitHead(hash common.Hash) error {
	...
	if block := dl.GetBlockByHash(hash); block != nil {
		_, err := trie.NewSecure(block.Root(), trie.NewDatabase(dl.stateDb.GlobalTable()))
		return err
	}
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	go func() {
		...
		if err := tester.sync("peer-half", nil, mode); err != nil {
			errCh <- fmt.Errorf("failed to synchronise blocks: %v", err)
		}
	}()
	<-starting
...
func testFailedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	go func() {
		...
		if err := tester.sync("faulty", nil, mode); err == nil {
			errCh <- fmt.Errorf("succeeded faulty synchronisation")
		}
	}()
	<-starting
	checkProgress(t, tester.downloader, "initial", gochain.SyncProgress{
...
func testFakedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	go func() {
		defer close(errCh)
		if err := tester.sync("attack", nil, mode); err == nil {
			errCh <- errors.New("succeeded attacker synchronisation")
		}
	}()
	<-starting
	checkProgress(t, tester.downloader, "initial", gochain.SyncProgress{
