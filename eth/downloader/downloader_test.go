...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	go func() {
		...
		if err := tester.sync(ctx, "peer-half", nil, mode); err != nil {
			syncErr = fmt.Errorf("failed to synchronise blocks: %v", err)
		}
	}()
	<-starting
...
func testFailedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	go func() {
		...
		if err := tester.sync(ctx, "faulty", nil, mode); err == nil {
			syncErr = errors.New("succeeded faulty synchronisation")
		}
	}()
	<-starting
	checkProgress(t, tester.downloader, "initial", gochain.SyncProgress{
...
func testFakedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	go func() {
		defer pending.Done()
		if err := tester.sync(ctx, "attack", nil, mode); err == nil {
			syncErr = errors.New("succeeded attacker synchronisation")
		}
	}()
	<-starting
	checkProgress(t, tester.downloader, "initial", gochain.SyncProgress{
...
func TestDeliverHeadersHang(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("protocol %d mode %v", tc.protocol, tc.syncMode), func(t *testing.T) {
			t.Parallel()
			testDeliverHeadersHang(context.Background(), t, tc.protocol, tc.syncMode)
		})
	}
}

func testDeliverHeadersHang(ctx context.Context, t *testing.T, protocol int, mode SyncMode) {
	master := newTester()
	...
	for i := 0; i < cap(deliveriesDone); i++ {
		...
		case <-deliveriesDone:
			if !launched {
				...
				go func() {
					ftp.peer.RequestHeadersByNumber(ctx, from, count, skip, reverse)
					deliveriesDone <- struct{}{}
				}()
				launched = true
			}
